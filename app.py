...
def bp_normal_label(predicted_ap_hi, ap_lo):
    """Binary status for the regression output: Normal or Tidak Normal."""
...
def get_feature_importance_df(model, feature_names, input_df=None, top_n=6):
    """
    Simple Explainable AI using feature importance from the trained model.
    This is lightweight and suitable for Streamlit deployment.
    """
...
def render_xai_section(title, model, feature_names, input_df, top_n=6):
    ...
    st.write(
        ...
        "Nilainya berasal dari feature importance model, jadi ini membantu menjelaskan pola model, "
        "bukan berarti menjadi penyebab medis secara langsung."
    )
...
def predict_full_profile(profile):
    ...
    reg_status = bp_normal_label(predicted_ap_hi, profile["ap_lo"])
...
with st.expander("Info performa model dan aturan deployment"):
    ...
    with col_c:
        ...
        st.metric("Aturan Normal", "<120 / <80")
    ...
    st.info(
        ...
        "Status regresi: Normal jika prediksi sistolik <120 mmHg dan diastolik <80 mmHg; selain itu Tidak Normal. "
...
with tab2:
    ...
    if st.button("Prediksi Tekanan Sistolik", type="primary"):
        predicted_ap_hi = predict_systolic(input_reg)
        status = bp_normal_label(predicted_ap_hi, ap_lo_r)
        ...
        st.caption(
            ...
            "Output ini hanya untuk kebutuhan deployment/tugas."
        )
...
with tab3:
    ...
    if st.button("Tampilkan Feature Paling Berpengaruh", type="primary"):
        ...
        predicted_ap_hi = predict_systolic(input_reg)
        status = bp_normal_label(predicted_ap_hi, profile["ap_lo"])
        ...
        st.caption(
            "Nilai feature importance menjelaskan pola yang dipakai model saat membuat prediksi. "
            "Ini bukan diagnosis medis dan bukan bukti penyebab langsung."
        )
